...
function constructPartsAndArgs(opts) {
  ...
  `;
	...
	else args.push(require_objectSpread2._objectSpread2({
		input,
		result: opts.result,
...
const defaultLogger = ({ c = console, colorMode = "css", withContext }) => (props) => {
	...
	const { parts, args } = constructPartsAndArgs(require_objectSpread2._objectSpread2(require_objectSpread2._objectSpread2({}, props), {}, {
		colorMode,
		input,
		withContext
	}));
	...
	return () => {
		return ({ op, next }) => {
			return (0, _trpc_server_observable.observable)((observer) => {
				if (enabled(require_objectSpread2._objectSpread2(require_objectSpread2._objectSpread2({}, op), {}, { direction: "up" }))) logger(require_objectSpread2._objectSpread2(require_objectSpread2._objectSpread2({}, op), {}, { direction: "up" }));
				const requestStartTime = Date.now();
				function logResult(result) {
					const elapsedMs = Date.now() - requestStartTime;
					if (enabled(require_objectSpread2._objectSpread2(require_objectSpread2._objectSpread2({}, op), {}, {
						direction: "down",
						result
					}))) logger(require_objectSpread2._objectSpread2(require_objectSpread2._objectSpread2({}, op), {}, {
						direction: "down",
						elapsedMs,
						result
					}));
				}
				return next(op).pipe((0, _trpc_server_observable.tap)({
					next(result) {
						logResult(result);
					},
					error(result) {
